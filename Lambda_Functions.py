...
#list of all the first names of the students who are sophomores.
sophNames = [i[0] for i in studentPerf.keys() if i[2]=="sophomore"]

#a unique list of all the different values listed in the key tuple in the index position 2


classes = list(set(select(studentPerf.keys(),2)))


#Using the select() function to do a similar task to the one above, but only for the key elements in the Index 1 position.
genders = list(set(select(studentPerf.keys(),1)))

#dictionary with the same keys as in studentPerf and values that represent the average of each student’s four test grades

dictAvgGrade = {}
for i in studentPerf.keys():
    dictAvgGrade[i]=(studentPerf[i][1]+studentPerf[i][2]+studentPerf[i][2]+studentPerf[i][3])/4

#data from the dictAvgGrade dictionary and create a list of the average grades for all sophomores

gradesSoph =[dictAvgGrade[i] for i in dictAvgGrade.keys() if i[2]=="sophomore"]


#class names as keys and values as the no of students in that class


dictDemClass ={i:0 for i in classes}
for i in list(studentPerf.keys()):
    dictDemClass[i[2]]=dictDemClass[i[2]]+1

#genders as the keys and values as the no of students in that gender


dictDemGender ={i:0 for i in genders}
for i in list(studentPerf.keys()):
    dictDemGender[i[1]]=dictDemGender[i[1]]+1

#class names as the keys and values as the average of the students in that respective class

dictGradeClass ={"sophomore":0,"junior":0,"senior":0}
for i in dictAvgGrade.keys():
    dictGradeClass[i[2]]=dictGradeClass[i[2]]+dictAvgGrade[i]
for i in dictGradeClass.keys():
    dictGradeClass[i]=dictGradeClass[i]/dictDemClass[i]
